...
| U3 | U | Conversation search | Ctrl+F over the chat history with highlight + next/prev. |
...
| **P15** | **P** | **Room-map UI — `gui/room_map_panel.py` + View menu** | Standalone `QGraphicsView`-backed window showing a top-down plan view: camera at origin, field-of-view cone, persistent dots for each detected/tracked object labelled with class + distance, motion trails for items the LLM has under active `track_object`, the user's face direction (head-pose yaw) as a heading arrow. Accessed from **View → Room map…** with shortcut `Ctrl+Shift+Z`. Subscribes to `ROOM_MAP` for updates. Distance reported in "rel units" by default; switches to metres once P16 calibration is done. |
| **P16** | **P** | **Camera intrinsics calibration** | Optional utility: hold a credit card / known-size object at a known distance; capture; compute metric scale so all room-map distances become real metres. Persisted to `.faceview/camera_calibration.json` so it survives restart. Surface as a "Calibrate camera…" button in the Map window. |
| P17 | P | LLM tool: `describe_room_layout()` | Once P14 ships, expose a tool the chat bot can call to get a one-paragraph natural-language description ("there's a coffee cup 80 cm to your left, a laptop 1.2 m ahead, a person sitting directly in front of the camera"). Reads from `RoomMapStore` — no new VLM call. |
